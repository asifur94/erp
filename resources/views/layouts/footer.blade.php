<div class="bg-white  p-6 shadow-sm">
    <div class="mx-auto" >
      <div class="flex flex-wrap flex-row -mx-4" >
        <div class="flex-shrink max-w-full px-4 w-full md:w-1/2 text-center md:ltr:text-left md:rtl:text-right" >
          <ul class="ltr:pl-0 rtl:pr-0">
            <li class="inline-block ltr:mr-3 rtl:ml-3">
              <a class="hover:text-indigo-500" href="#">Support</a>
            </li>

          </ul>
        </div>
        <div class="flex-shrink max-w-full px-4 w-full md:w-1/2 text-center md:ltr:text-right md:rtl:text-left" bis_skin_checked="1">
          <p class="mb-0 mt-3 md:mt-0">
            <a href="http://insomniacafe.org/" class="hover:text-indigo-500">Insomniacafe</a> | All right reserved
          </p>
        </div>
      </div>
    </div>
</div>
